...
export const authLogin = async (email, password, callback) => {
  ...
  try {
    const response = await axios.post(
      `https://hrappbackendwebapi20240119093404.azurewebsites.net/api/Account/Login`,
      {
        email,
        password,
      }
...
export const getLogo = () => async (dispatch) => {
  try {
    const { data } = await axios.get(
      'https://hrappbackendwebapi20240119093404.azurewebsites.net/api/Employee/GetCompanyLogo'
    );
